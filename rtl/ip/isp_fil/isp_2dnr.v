...
/*	input [7*7*WEIGHT_BITS-1:0] space_kernel, //空域卷积核(7x7)
	...
	localparam [9*BITS-1:0]				color_curve_x={8'd30, 8'd26, 8'd23, 8'd20, 8'd17, 8'd13, 8'd10, 8'd6 , 8'd3 };
	localparam [9*WEIGHT_BITS-1:0]		color_curve_y={5'd0 , 5'd1 , 5'd2 , 5'd4 , 5'd7 , 5'd13, 5'd19, 5'd26, 5'd30};
	localparam [7*7*WEIGHT_BITS-1:0]	space_kernel=
	...
	always @ (posedge pclk) begin : _blk_p_t1
		integer i, j;
		if (rst) begin
			in_data_r <= 0;
			for (i = 0; i < 7*7; i = i + 1)
	...
	always @ (posedge pclk) begin : _blk_p_t2
		integer i, j;
		if (rst) begin
	...
	always @ (posedge pclk) begin : _blk_diff_t2
		integer i, j;
		if (rst) begin
			for (i = 0; i < 7*7; i = i + 1)
				diff_t2[i] <= 0;
	...
	always @ (posedge pclk) begin : _blk_color_weight_t3
		integer i, j;
		if (rst) begin
			for (i = 0; i < 7*7; i = i + 1)
				color_weight_t3[i] <= 0;
	...
	always @ (posedge pclk) begin : _blk_weight_t4
		integer i, j;
		if (rst) begin
			for (i = 0; i < 7*7; i = i + 1)
				weight_t4[i] <= 0;
	...
	always @ (posedge pclk) begin : _blk_value_mul_t5
		integer i, j;
		if (rst) begin
			for (i = 0; i < 7*7; i = i + 1)
				value_mul_t5[i] <= 0;
	...
	always @ (posedge pclk) begin : _blk_value_sum_x_t6
		integer i;
		if (rst) begin
			for (i = 0; i < 7; i = i + 1)
				value_sum_x_t6[i] <= 0;
	...
	always @ (posedge pclk) begin : _blk_weight_sum_x_t5
		integer i;
		if (rst) begin
			for (i = 0; i < 7; i = i + 1)
				weight_sum_x_t5[i] <= 0;
	...
	shift_div_uint #(BITS+2*WEIGHT_BITS+6) target_div (pclk, rst, value_sum_t7, {{BITS{1'b0}},weight_sum_t7}, target_quo, target_rem);
	...
	always @ (posedge pclk) begin
		if (rst) begin
			...
			clken_dly <= {clken_dly[DLY_CLK-2:0], in_clken};
		end
	end
	
	assign out_href = href_dly[DLY_CLK-1];
	assign out_vsync = vsync_dly[DLY_CLK-1];
	assign out_clken = clken_dly[DLY_CLK-1];
